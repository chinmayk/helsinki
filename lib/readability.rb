...
module Readability
  class Document
    ...
    def content(remove_unlikely_candidates = :default)
    	    found_content = self._content
    	    
    	    if found_content.content.length < 300 
    	    	    headers_length = found_content.css('h1,h2,h3,h4,h5,h6').reduce(0) do |acc, e|
					acc + e.content.length
				end
				if headers_length > 0.5*found_content.content.length
					return Nokogiri::XML::Node.new('p',@html)
				else
    ...
    def _content(remove_unlikely_candidates = :default)
    ...
    def sanitize(node, candidates, options = {})
      ...
      node.css("form, object, iframe, embed").each do |elem|
        elem.remove
    	    ...
    	    #Do we really need to remove these headers? CEK
      ...
      #Am not sure if the headers below will have any effect, given they were removed above. CEK
      base_replace_with_whitespace = %w[br hr h1 h2 h3 h4 h5 h6 dl dd ol li ul address blockquote center div]
      ...
      #Not using the below code. Does more harm than good.
      ...
      node.css(selector).each do |el|
        ...
        if weight + content_score < 0
          ...
          if counts["img"] > counts["p"]
            ...
            to_remove = true
    elsif content_length < (options[:min_text_length] || TEXT_LENGTH_THRESHOLD) && (counts["img"] == 0 || counts["img"] > 2) && (counts["li"] == 0)
            reason = "too short a content length without a single image"
            to_remove = true
          elsif weight < 25 && link_density > 0.2
            reason = "too many links for its weight (#{weight})"
            to_remove = true
          elsif weight >= 25 && link_density > 0.5 && counts["li"] == 0
            reason = "too many links for its weight (#{weight})"
            to_remove = true
